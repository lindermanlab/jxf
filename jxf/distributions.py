...
class LinearRegression(ExponentialFamilyDistribution):
    ...
    def sample(self, rng, covariates, sample_shape=()):
...
@register_pytree_node_class
class LinearAutoRegression(ExponentialFamilyDistribution):
    """A linear regression model. The optimal weights are computed
    via the expected sufficient statistics of the data.
    """
    ...
    def sufficient_statistics(data,
        ...
        if covariates is not None:
            assert covariates.shape[-1] == covariate_dim
        ...
        return (np.einsum('...i,...j->...ij', all_covariates, all_covariates),
                np.einsum('...i,...j->...ij', data, all_covariates),
                np.einsum('...i,...j->...ij', data, data))
...
# Also register the distribution objects as PyTrees for JAX support
# for typ in [Bernoulli,
#             Beta,
#             Binomial,
#             Categorical,
#             Dirichlet,
...
#     # There is some weird bug in the closure of `typ` in the reconstruct function
#     register_pytree_node(
#         typ,
#         lambda value: (value.parameters.items(), None),
#         lambda aux_data, params: typ(**dict(params))
#     )

register_pytree_node(
        Bernoulli,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: Bernoulli(**dict(params))
    )

register_pytree_node(
        Beta,
        lambda value: (dict(value.parameters.items(), total_count=value.total_count), None),
        lambda aux_data, params: Beta(**dict(params))
    )

register_pytree_node(
        Binomial,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: Binomial(**dict(params))
    )

register_pytree_node(
        Categorical,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: Categorical(**dict(params))
    )

register_pytree_node(
        Dirichlet,
        lambda value: (dict(value.parameters.items(), total_count=value.total_count), None),
        lambda aux_data, params: Dirichlet(**dict(params))
    )

register_pytree_node(
        Gamma,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: Gamma(**dict(params))
    )

register_pytree_node(
        Multinomial,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: Multinomial(**dict(params))
    )

register_pytree_node(
        MultivariateNormalFullCovariance,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: MultivariateNormalFullCovariance(**dict(params))
    )

register_pytree_node(
        MultivariateNormalTriL,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: MultivariateNormalTriL(**dict(params))
    )

register_pytree_node(
        Normal,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: Normal(**dict(params))
    )

register_pytree_node(
        Poisson,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: Poisson(**dict(params))
    )

register_pytree_node(
        StudentT,
        lambda value: (value.parameters.items(), None),
        lambda aux_data, params: StudentT(**dict(params))
    )
